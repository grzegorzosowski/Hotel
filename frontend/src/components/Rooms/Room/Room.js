...
function Room(props) {
    return (
        <>
        <Box sx={{padding: '75px 20px', height:'600px',  color: 'white', display: 'flex', justifyContent: 'space-evenly'}}>
            <Box sx={{width: '40%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: "center"}}>
                <Typography variant="h3" mb={'20px'}>{props.name}</Typography>
                <Typography variant="body1" my='10px'><b>Class:</b> {props.type}</Typography>
